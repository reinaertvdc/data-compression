...
public:
    ...
    int16_t getValue(int row, int col) const { if (this->empty) return static_cast<int16_t>(0); else return this->matrix[row][col]; }
    short getValueAsShort(int row, int col) const { if (this->empty) return static_cast<short>(0); else return static_cast<short>(this->matrix[row][col]); }
