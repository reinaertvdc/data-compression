...
uint8_t *StorageFormatCodec::toStorageFormat(int16_t *data, int size, int &outSize, int width, int height, bool rle,
    ...
    uint8_t wh[4] = {static_cast<uint8_t>(width / 4 / 256), static_cast<uint8_t>(width / 4),
                     static_cast<uint8_t>(height / 4 / 256), static_cast<uint8_t>(height / 4)};
...
int16_t *
StorageFormatCodec::fromStorageFormat(uint8_t *data, int size, int &outSize, int &wOut, int &hOut, bool &rleOut,
                                      ValueBlock4x4 &quant) {
